...
for order, ratio in enumerate(ratio_file):
    ...
    ax1.set_xlabel('Wavelength($\AA$)', color='black')
    _ = ax1.plot(wavelength, ratio, '-',
                 wavelength, fitted(pixels), '--')
    ax1.tick_params(axis='x', labelcolor='black')
    ax2 = ax1.twiny()
    ax2.set_xlabel('pixels', color='red')
    ax2.plot(pixels_actual[master_mask], ratio[master_mask], color='red')
    ax2.tick_params(axis='x', labelcolor='red')
    ax1.set_ylim(-1e14, max(ratio)+1000)
    ax2.set_ylim(-1e14, max(ratio)+1000)
