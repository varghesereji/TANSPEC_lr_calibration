...
for order, ratio in enumerate(ratio_file):
    ...
    for wl in telluric_lines:
        ax[0].axvline(x=wl, color='black', linestyle=':', alpha=0.2)
        ax[1].axvline(x=wl, color='black', linestyle=':', alpha=0.2)
    for wl in HYDROGEN:
        ax[1].axvline(x=wl, color='blue', linestyle='--')
    ...
    ax[1].set_xlim(min(wavelength[mask_portion]),
                   max(wavelength[mask_portion]))
    ax[0].set_xlim(min(wavelength[mask_portion]),
                   max(wavelength[mask_portion]))
    pdfplots.savefig()
