...
fig, ax1 = plt.subplots(5, 2, figsize=(16, 9))
fig.suptitle('Star: {0}, Slit: {1}'.format(star, slit))
for order, ratio in enumerate(ratio_file):
    ...
    color = 'red'
    ax1[order // 2, order % 2].set_xlabel('wavelength $\AA$')
    ax1[order // 2, order % 2].set_ylabel('TANSPEC Flux', color=color)
    ax1[order // 2, order % 2].plot(
        wavelength[mask_portion],
        calib_tanspec[mask_portion],
        color='red', label='TANSPEC Order {}'.format(12 - order))
    ax1[order // 2, order % 2].tick_params(axis='y', labelcolor=color)
    ax2 = ax1[order // 2, order % 2].twinx()
    color = 'green'
    cal_orders = calspec_data(wavelength, star=calspec)
    scaling_factor = np.median(calib_tanspec / cal_orders)
    cal_orders = cal_orders * scaling_factor
    adding_factor = np.median(calib_tanspec[mask_portion]
                              - cal_orders[mask_portion])
    ax2.plot(wavelength[mask_portion], cal_orders[mask_portion]+adding_factor,
             color='green', label='CALSPEC')
    ...
    ax1[order // 2, order % 2].set_xlim(min(wavelength[mask_portion]),
                                        max(wavelength[mask_portion]))
    ax1[order // 2, order % 2].set_ylim(min(cal_orders),
                                        max(cal_orders))
    ax1[order // 2, order % 2].legend(loc='upper right')
    # ax2.legend()
plt.savefig('All_orders_{0}_{1}.png'.format(star, slit))
# plt.show()
