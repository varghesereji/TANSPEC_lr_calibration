...
for order, ratio in enumerate(ratio_file):
    ...
    axs[0].plot(wavelength[mask_portion], ratio[mask_portion],
                label='{}'.format(order))
    axs[1].plot(wavelength[mask_portion], calib_tanspec[mask_portion],
                )
    cal_orders = calspec_data(wavelength, star=calspec)
    axs[1].plot(wavelength[mask_portion], cal_orders[mask_portion],
                color='green')
    print(min(wavelength[mask_portion]), max(wavelength[mask_portion]))
axs[1].set_ylim(1.5e-14, 4.0e-12)
axs[0].legend()
plt.show()
