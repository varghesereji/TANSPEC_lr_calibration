...
scale = np.array([6, 6, 6, 6, 6, 6, 6, 6, 6, 6])
...
for order, ratio in enumerate(ratio_file):
    ...
    color = 'green'
    cal_orders = calspec_data(wavelength, star=calspec) / scale[order]
    ...
    ax1[order // 2, order % 2].set_xlim(min(wavelength[mask_portion]),
                                        max(wavelength[mask_portion]))
    ax1[order // 2, order % 2].set_ylim(min(cal_orders) * scaling_factor,
