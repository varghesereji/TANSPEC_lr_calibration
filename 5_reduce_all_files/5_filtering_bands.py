    ...
    for order in range(0, 10, 1):
        ...
        if filter_dict != {}:
            ...
            for q, band in enumerate(band_name):
                ...
                fig.suptitle(
                    '{0} Order {1} band {2}'.format(star, order, band))
                ...
                axs[2].plot(wavelength[mask], cal_orders[mask], color='green',
                            label='calspec')
                axs[1].plot(wavelength[mask],
                            corrected_flux[mask], color='red',
                            label='calibrated tanspec')
                axs[3].plot(wavelength[mask], cal_orders[mask], color='green',
                            label='calspec')
                axs[3].plot(wavelength[mask],
                            corrected_flux[mask], color='red',
                            label='calibrated tanspec')
            axs[0].legend()
            axs[1].legend()
            axs[2].legend()
            axs[3].legend()
            print(min(cal_orders[mask]), max(cal_orders[mask]))
            axs[3].set_ylim(min(cal_orders[mask]), max(cal_orders[mask]))
